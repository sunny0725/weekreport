...
export function buildPlaceholders(
  ...
  return {
    ...
    KPI_RATE:           kpiRate,
    THIS_WEEK_DETAIL:   input.thisWeekWork.map((item, i) => `${i + 1}. ${item}`).join("\n"),
    NEXT_WEEK_DETAIL:   input.nextWeekPlan.map((item, i) => `${i + 1}. ${item}`).join("\n"),
    ISSUES_DETAIL:      input.issues.map((item, i) => `${i + 1}. ${item}`).join("\n"),
  };
}
...
export async function generateHwpxReport(input: WeeklyReportInput): Promise<Buffer> {
  ...
  try {
    ...
    throw new Error(
      "템플릿 파일이 없습니다. templates/weekly_report_template.hwpx 를 준비해주세요.\n" +
      "한글 오피스에서 기존 .hwp 파일을 열고 [다른 이름으로 저장 → .hwpx]로 변환 후,\n" +
